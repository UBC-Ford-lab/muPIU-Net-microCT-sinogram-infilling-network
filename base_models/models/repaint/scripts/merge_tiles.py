#!/usr/bin/env python3
"""
Merge RePaint Infilled Tiles Back to Full Resolution
====================================================
This script takes 256×256 infilled tiles from RePaint and merges them back
into full-resolution (416×3504) sinograms with proper overlap blending.
...
The reconstruction script (reconstruct_from_repaint.py) will denormalize using
the per-sinogram min/max values stored in metadata.

Author: Claude (Anthropic)
Date: 2025-11-18 (updated 2025-12-04: fixed double denormalization bug)
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Merge RePaint tiles back to full-resolution sinograms'
    )
    parser.add_argument(
        '--tiles_dir',
        type=str,
        default='../data/tiles_infilled',
        help='Input directory containing infilled tiles from RePaint'
    )
    parser.add_argument(
        '--metadata_path',
        type=str,
    ...
    parser.add_argument(
        ...
        default='gaussian',
        help='Blending method for overlaps: linear, gaussian, or nearest (no blending, default: gaussian)'
    )
    parser.add_argument(
        '--gaussian_sigma',
        type=float,
...
def main():
    ...
    for sino_idx in tqdm(range(n_sinograms), desc="Merging sinograms"):
        ...
        for tile_idx in range(n_tiles):
            tile_name = f"sino_{sino_idx:04d}_tile_{tile_idx:02d}.png"  # 16-bit PNG
            tile_path = tiles_dir / tile_name

            if not tile_path.exists():
                raise FileNotFoundError(f"Missing tile: {tile_path}")

            # Load 16-bit grayscale PNG (PIL supports this natively)
            ...
            # Tiles should be grayscale uint16 (or uint8 for backward compat)
            # (RePaint processes RGB internally but saves as grayscale)
    ...
    sample_path = output_dir / "sino_0000.png"
    ...
    print(f"\nNext steps:")
    print(f"  python reconstruct_from_repaint.py")
