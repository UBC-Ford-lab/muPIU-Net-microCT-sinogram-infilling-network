#!/usr/bin/env python3
"""
Reconstruction Pipeline for RePaint-Infilled Sinograms
======================================================
This script:
1. Loads RePaint-infilled (merged) sinogram PNGs using memory-mapped arrays (RAM-safe)
...
Based on reconstruct_from_lama.py structure
"""
...
def load_infilled_sinograms(sinogram_folder, metadata, memmap_path, force_reload=False):
    ...
    for idx, sino_file in enumerate(tqdm(sinogram_files, desc="Loading sinograms")):
        ...
        # NOTE: As of 2025-11-24, merge_repaint_tiles.py now removes padding automatically
        # So input sinograms are already in original shape (410×3500), no padding removal needed
        img_unpadded = img
        ...
        if img_unpadded.shape != (n_projections, original_width):
            ...
            print(f"  This sinogram was already padding-removed by merge_repaint_tiles.py")
            print(f"  If using old sinograms (with padding), you need to re-run merging")
            continue
...
def reconstruct_from_repaint_output(
    infilled_folder='../data/sinograms_infilled',
    metadata_path='../../../shared/sinogram_dataset/metadata.json',
    tiling_metadata_path='../data/sinogram_tiles/tiling_metadata.json',
    ...
    """
    Main reconstruction pipeline for RePaint-infilled sinograms

    Args:
        infilled_folder: Path to RePaint merged output sinograms
    ...
    print("="*70)
    print("RePaint Sinogram Reconstruction Pipeline")
    print("="*70)
    ...
    if memmap_path is None:
        ...
        memmap_path = str(output_folder.parent / 'repaint_projections_memmap.dat')
    ...
    working_memmap_path = str(Path(memmap_path).parent / 'repaint_projections_working.dat')
    ...
    # The missing projections (odd indices) were infilled by RePaint
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Reconstruct CT volumes from RePaint-infilled sinograms'
    )
    parser.add_argument(
        '--sinogram_dir',
        type=str,
    ...
    # Handle paths: if relative, use as-is (relative to CWD); if absolute, use as-is
    # Script is run from Base_model_comparison/, so relative paths are relative to that
    ...
    # Reconstruct RePaint infilled sinograms
    print("\n" + "="*80)
    print("RECONSTRUCTING REPAINT INFILLED")
    print("="*80)
    reconstruct_from_repaint_output(
    ...
    print("="*80)
    print(f"✓ RePaint reconstruction: {output_dir}.vff")
