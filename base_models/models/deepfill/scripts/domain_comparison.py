#!/usr/bin/env python3
"""
DeepFill v2 Infilling Domain Comparison Script
===============================================

This script computes SSIM and PSNR metrics for DeepFill v2-based sinogram infilling
...
Compares:
- Sinogram domain: GT sinograms vs DeepFill v2 infilled sinograms
- Reconstruction domain: GT reconstruction vs DeepFill v2 reconstruction vs Projection infilling reconstruction

Written for CT reconstruction base model comparison.
Based on repaint_domain_comparison.py structure.
...
def parse_args():
    """Parse command line arguments with sensible defaults for DeepFill v2 comparison."""
    p = argparse.ArgumentParser(
        description="Compare SSIM and PSNR across sinogram and reconstruction domains for DeepFill v2 infilling"
    )
    p.add_argument(
        '--gt_sinogram_folder',
        type=str,
    ...
    p.add_argument(
        '--deepfill_sinogram_folder',
        type=str,
        default='../data/sinograms_infilled',
        help='Folder containing DeepFill v2 infilled sinograms (PNG files)'
    )
    p.add_argument(
        '--gt_recon_file',
        type=str,
    ...
    p.add_argument(
        ...
        help='DeepFill v2 reconstruction VFF file'
    )
    p.add_argument(
...
class SinogramDataset(Dataset):
    """Dataset for loading sinogram pairs (GT and DeepFill v2 infilled)"""

    def __init__(self, gt_folder, deepfill_folder):
        """
        Initialize sinogram dataset.

        Args:
            gt_folder: Path to ground truth sinogram folder
            deepfill_folder: Path to DeepFill v2 infilled sinogram folder
        """
        if not os.path.exists(gt_folder):
            raise ValueError(f"GT sinogram folder does not exist: {gt_folder}")
        if not os.path.exists(deepfill_folder):
            raise ValueError(f"DeepFill v2 sinogram folder does not exist: {deepfill_folder}")
        ...
        self.deepfill_folder = deepfill_folder

        # Check for shape mismatch (GT is padded, DeepFill is original shape)
        gt_sample = np.array(Image.open(os.path.join(gt_folder, self.gt_files[0])))
        df_sample = np.array(Image.open(os.path.join(deepfill_folder, self.gt_files[0])))

        if gt_sample.shape != df_sample.shape:
            print(f"  Note: Shape mismatch detected - GT {gt_sample.shape} vs DeepFill {df_sample.shape}")
            print(f"  GT will be center-cropped to match DeepFill dimensions")
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            Tuple of (filename, gt_sinogram, deepfill_sinogram)
        """
        gt_fname = self.gt_files[idx]

        # DeepFill outputs use the same filename as GT
        deepfill_fname = gt_fname
        deepfill_path = os.path.join(self.deepfill_folder, deepfill_fname)
        ...
        # Read DeepFill infilled sinogram
        try:
            deepfill_img = Image.open(deepfill_path)
            deepfill = np.array(deepfill_img, dtype=np.float32)
        except Exception as e:
            raise ValueError(f"Error reading DeepFill sinogram {deepfill_path}: {e}")

        # Handle shape mismatch: GT is padded (416, 3504), DeepFill is original (410, 3500)
        if gt.shape != deepfill.shape:
            gt_h, gt_w = gt.shape
            df_h, df_w = deepfill.shape

            # Calculate crop offsets (center crop)
            h_offset = (gt_h - df_h) // 2
            w_offset = (gt_w - df_w) // 2

            # Only crop if GT is larger than DeepFill (expected case)
            if gt_h >= df_h and gt_w >= df_w:
                gt = gt[h_offset:h_offset + df_h, w_offset:w_offset + df_w]
            else:
                raise ValueError(
                    f"Unexpected shape mismatch for {gt_fname}: GT {gt.shape} vs DeepFill {deepfill.shape}. "
                    f"Expected GT to be larger or equal."
                )

        return gt_fname, gt, deepfill


def compute_sinogram_domain_metrics(gt_folder, deepfill_folder):
    """
    ...
    print(f"  DeepFill v2 folder: {deepfill_folder}")

    ssim_values = []
    psnr_values = []

    try:
        ds = SinogramDataset(gt_folder, deepfill_folder)
        dl = DataLoader(ds, batch_size=None, num_workers=4, pin_memory=True)

        for fname, gt, deepfill in tqdm(dl, total=len(ds), desc="  Computing metrics"):
            if isinstance(gt, np.ndarray):
                gt_tensor = torch.from_numpy(gt)
            else:
                gt_tensor = gt

            if isinstance(deepfill, np.ndarray):
                deepfill_tensor = torch.from_numpy(deepfill)
            else:
                deepfill_tensor = deepfill

            ssim_val = compute_ssim(gt_tensor, deepfill_tensor)
            psnr_val = compute_psnr(gt_tensor, deepfill_tensor)
...
def compute_reconstruction_domain_metrics(
    ...
    Args:
        ...
        deepfill_recon_file: Path to DeepFill v2 reconstruction VFF file
        projection_infilling_recon_file: Optional path to projection infilling reconstruction VFF file

    Returns:
        Dictionary with 'deepfill_ssim', 'deepfill_psnr', and optionally 'projection_ssim', 'projection_psnr'
    """
    ...
    # Compute metrics for DeepFill v2 reconstruction
    if os.path.exists(deepfill_recon_file):
        print("  Loading DeepFill v2 reconstruction...")
        try:
            _, deepfill_recon = vff_io.read_vff(deepfill_recon_file, verbose=False)
            deepfill_recon = deepfill_recon.byteswap().view(deepfill_recon.dtype.newbyteorder()).astype(np.float32)
            print(f"  DeepFill v2 reconstruction shape: {deepfill_recon.shape}")

            if gt_recon.shape == deepfill_recon.shape:
                print(f"  Computing DeepFill v2 SSIM and PSNR for {gt_recon.shape[0]} slices...")
                deepfill_ssim_values = []
                deepfill_psnr_values = []

                for slice_idx in tqdm(range(gt_recon.shape[0]), desc="  DeepFill v2 metrics"):
                    gt_slice = torch.from_numpy(gt_recon[slice_idx])
                    deepfill_slice = torch.from_numpy(deepfill_recon[slice_idx])

                    ssim_val = compute_ssim(gt_slice, deepfill_slice)
                    psnr_val = compute_psnr(gt_slice, deepfill_slice)
                    deepfill_ssim_values.append(ssim_val)
                    deepfill_psnr_values.append(psnr_val)

                results['deepfill_ssim'] = np.array(deepfill_ssim_values)
                results['deepfill_psnr'] = np.array(deepfill_psnr_values)
            else:
                print(f"  Warning: Shape mismatch - GT: {gt_recon.shape}, DeepFill: {deepfill_recon.shape}")
        except Exception as e:
            print(f"  Error processing DeepFill v2 reconstruction: {e}")
    else:
        print(f"  Warning: DeepFill v2 reconstruction file not found: {deepfill_recon_file}")
...
def save_results_to_file(
    ...
    output_file = os.path.join(output_dir, 'deepfill_metrics_summary.txt')

    with open(output_file, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("DEEPFILL v2 INFILLING DOMAIN COMPARISON RESULTS\n")
        f.write("=" * 80 + "\n\n")
        ...
        # DeepFill v2 reconstruction
        if 'deepfill_ssim' in recon_metrics:
            deepfill_ssim = recon_metrics['deepfill_ssim']
            f.write(f"DeepFill v2 SSIM: mean={deepfill_ssim.mean():.4f}, std={deepfill_ssim.std():.4f}, ")
            f.write(f"min={deepfill_ssim.min():.4f}, max={deepfill_ssim.max():.4f}, count={len(deepfill_ssim)}\n")
        if 'deepfill_psnr' in recon_metrics:
            deepfill_psnr = recon_metrics['deepfill_psnr']
            f.write(f"DeepFill v2 PSNR: mean={deepfill_psnr.mean():.2f} dB, std={deepfill_psnr.std():.2f} dB, ")
            f.write(f"min={deepfill_psnr.min():.2f} dB, max={deepfill_psnr.max():.2f} dB, count={len(deepfill_psnr)}\n")
        f.write("\n")
...
def create_comparison_plots(
    ...
    Args:
        ...
        deepfill_sino_folder: Path to DeepFill v2 sinogram folder
        gt_recon_file: Path to GT reconstruction VFF
        deepfill_recon_file: Path to DeepFill v2 reconstruction VFF
    ...
    try:
        ds = SinogramDataset(gt_sino_folder, deepfill_sino_folder)
        if len(ds) > 0:
            middle_idx = len(ds) // 2
            fname, gt_sino, deepfill_sino = ds[middle_idx]
            ...
            # DeepFill sinogram
            im2 = ax2.imshow(deepfill_sino, cmap='gray', aspect='auto', interpolation='bilinear')
            ax2.set_title('DeepFill v2 Infilled Sinogram', fontweight='bold')
            ax2.set_xlabel('Detector Width')
            ax2.set_ylabel('Projection Angle')
            plt.colorbar(im2, ax=ax2, fraction=0.046, pad=0.04)

            plt.tight_layout()
            sino_plot_path = os.path.join(output_dir, 'deepfill_sinogram_comparison.png')
    ...
    try:
        if os.path.exists(gt_recon_file):
            ...
            if os.path.exists(deepfill_recon_file):
                _, deepfill_recon = vff_io.read_vff(deepfill_recon_file, verbose=False)
                deepfill_recon = deepfill_recon.byteswap().view(deepfill_recon.dtype.newbyteorder()).astype(np.float32)
                deepfill_slice = deepfill_recon[central_slice_idx]

                im2 = axes[plot_idx].imshow(deepfill_slice, cmap='gray', interpolation='bilinear')
                axes[plot_idx].set_title('DeepFill v2 Reconstruction', fontweight='bold')
                axes[plot_idx].axis('off')
                plot_idx += 1
            ...
            plt.tight_layout()
            recon_plot_path = os.path.join(output_dir, 'deepfill_reconstruction_comparison.png')
...
def main():
    ...
    print("=" * 80)
    print("DEEPFILL v2 INFILLING DOMAIN COMPARISON ANALYSIS")
    print("=" * 80)
    ...
    if not os.path.exists(args.deepfill_sinogram_folder):
        raise FileNotFoundError(f"DeepFill v2 sinogram folder not found: {args.deepfill_sinogram_folder}")
    ...
    elif 'deepfill_psnr' in recon_metrics:
        original_count = len(recon_metrics['deepfill_psnr'])
        filter_mask = recon_metrics['deepfill_psnr'] < 26
        n_kept = filter_mask.sum()
        n_removed = original_count - n_kept

        print(f"  Filter: PSNR < 26 dB (using DeepFill v2 PSNR)")
        print(f"  Slices kept: {n_kept}/{original_count} ({100*n_kept/original_count:.1f}%)")
        print(f"  Slices removed: {n_removed}/{original_count} ({100*n_removed/original_count:.1f}%)")
    ...
    print("-" * 80)
    if 'deepfill_ssim' in recon_metrics:
        print_statistics(recon_metrics['deepfill_ssim'], "Reconstruction (DeepFill v2)", "SSIM")
    if 'deepfill_psnr' in recon_metrics:
        print_statistics(recon_metrics['deepfill_psnr'], "Reconstruction (DeepFill v2)", "PSNR")
