#!/usr/bin/env python3
"""
Reconstruction Pipeline for LaMa-Infilled Sinograms
====================================================
This script:
1. Loads LaMa-infilled sinogram PNGs using memory-mapped arrays (RAM-safe)
...
Based on Scanner reconstruction main.py structure
"""
...
def load_metadata(metadata_path):
    """Load sinogram dataset metadata"""
...
def load_infilled_sinograms(sinogram_folder, metadata, memmap_path, force_reload=False):
    ...
    for idx, sino_file in enumerate(tqdm(sinogram_files, desc="Loading sinograms")):
        ...
        # Load PNG with preserved bit depth
        img_raw = cv2.imread(str(sino_file), cv2.IMREAD_UNCHANGED)

        if img_raw is None:
            print(f"\nWarning: Could not load {sino_file}, skipping")
            continue
        ...
        if img_raw.ndim == 3:
            # RGB/BGR - take first channel (all channels identical for grayscale-converted-to-RGB)
            img = img_raw[:, :, 0].astype(np.float32)
        else:
            # Already grayscale (new 16-bit workflow)
            img = img_raw.astype(np.float32)

        # Get sinogram metadata to extract actual padding values
        sino_metadata = metadata['sinograms'][height_idx]

        # Remove padding using the actual padding values from metadata
        # Padding format: [top, bottom, left, right]
        padding = sino_metadata['padding']
        top, bottom, left, right = padding

        # Remove padding: apply inverse of the padding
        if bottom == 0 and right == 0:
            img_unpadded = img[top:, left:]
        elif bottom == 0:
            img_unpadded = img[top:, left:-right]
        elif right == 0:
            img_unpadded = img[top:-bottom, left:]
        else:
            img_unpadded = img[top:-bottom, left:-right]
        ...
        # For 16-bit: max_value = 65535.0 (260x better precision than 8-bit)
        # For 8-bit: max_value = 255.0 (backward compatibility)
...
def reconstruct_from_lama_output(
    ...
    keep_memmap=True,  # Keep memmap by default for reuse
    memmap_path=None,  # Will auto-select based on available space
    force_reload=False  # If True, recreate memmap even if it exists
    ):
    """
    Main reconstruction pipeline

    Args:
        infilled_folder: Path to LaMa output sinograms
        ...
        keep_memmap: If True, keep memmap file after reconstruction (default: True for reuse)
        memmap_path: Path for temporary memory-mapped file (auto-selected if None)
                     Will use project directory instead of /tmp for better space
        force_reload: If True, recreate memmap even if it exists (default: False)
                      Subsequent runs will automatically reuse existing memmap for instant loading
    """
    start = time.time()
    ...
    print("="*70)
    print("LaMa Sinogram Reconstruction Pipeline")
    print("="*70)
    ...
    if memmap_path is None:
        # Use project directory instead of /tmp (more space available)
        memmap_path = str(output_folder.parent / 'lama_projections_memmap.dat')
    ...
    # This ensures we can reuse the cached sinogram data without corruption
    working_memmap_path = str(Path(memmap_path).parent / 'lama_projections_working.dat')
    ...
    # Generate angles
    # Assuming uniform angular spacing for the projections we HAVE (every other one)
    # The missing projections (odd indices) were infilled by LaMa
    ...
    #angles = torch.linspace(0, 2 * np.pi, n_projections, dtype=torch.float32)
    ...
    print(f"  Angular range: [0, 2π]")
    print(f"  Angular spacing: {(2 * np.pi / n_projections):.6f} rad ({360 / n_projections:.2f}°)")

    # Create output folder
    output_folder.mkdir(parents=True, exist_ok=True)

    # Initialize FDK reconstructor
    # Note: projections is already a numpy memmap array, pass directly
    ...
    try:
        reconstructor = FDKReconstructor(
            projections=projections,  # memmap array - FDKReconstructor already handles memmap
            angles=angles,
            geometry=geometry,
        ...
        # Always delete working copy (it's modified by FDK)
        try:
            del projections  # Close working memmap
        ...
        # Keep or delete the original cached sinogram data
        if keep_memmap:
            print(f"✓ Cached sinogram data preserved for reuse: {memmap_path}")
            print(f"  Size: {os.path.getsize(memmap_path) / (1024**3):.2f} GB")
            print(f"  Next run will reuse this file (instant loading)")
            print(f"  To force reload: pass force_reload=True")
        else:
            try:
                del projections_readonly  # Close original memmap
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Reconstruct CT volumes from sinogram datasets (LaMa infilled or GT)'
    )
    parser.add_argument(
        '--mode',
        type=str,
        choices=['gt', 'lama', 'both'],
        default='lama',
        help='Which reconstruction to perform: gt (ground truth), lama (LaMa infilled), or both'
    )
    parser.add_argument(
        '--scan_folder',
        type=str,
    ...
    if args.mode in ['gt', 'both']:
        ...
        reconstruct_from_lama_output(
            infilled_folder=script_dir / '../../../shared/sinogram_dataset/sinograms_gt',
            metadata_path=metadata_path,
            scan_folder=scan_folder,
            output_folder=script_dir / '../../../../data/results/gt_reconstruction',
            force_reload=args.force_reload,
            keep_memmap=True  # Keep for LaMa reconstruction if doing both
        )

    # Reconstruct LaMa if requested
    if args.mode in ['lama', 'both']:
        print("\n" + "="*80)
        ...
        reconstruct_from_lama_output(
            ...
            force_reload=args.force_reload if args.mode == 'lama' else False,  # Reuse memmap if doing both
            keep_memmap=True  # Keep for potential future runs
        )
    ...
    print("="*80)
    if args.mode in ['gt', 'both']:
        print(f"✓ GT reconstruction: {script_dir}/../../../../data/results/gt_reconstruction.vff")
    if args.mode in ['lama', 'both']:
        print(f"✓ LaMa reconstruction: {model_dir}/results/lama_reconstruction.vff")
