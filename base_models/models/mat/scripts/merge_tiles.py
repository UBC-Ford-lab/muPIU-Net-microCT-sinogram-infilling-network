#!/usr/bin/env python3
"""
Merge MAT Infilled Tiles Back to Full Resolution
=================================================
This script takes 256x256 infilled tiles from MAT and merges them back
into full-resolution (410x3500) sinograms with proper overlap blending.
...
The reconstruction script (reconstruct_from_mat.py) will denormalize using
the per-sinogram min/max values stored in metadata.

Based on merge_repaint_tiles.py and merge_deepfill_tiles.py structure.

Author: Claude (Anthropic)
Date: 2025-12-01 (updated 2025-12-05: fixed double denormalization bug)
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Merge MAT tiles back to full-resolution sinograms'
    )
    parser.add_argument(
        '--tiles_dir',
        type=str,
        default='../data/tiles_infilled',
        help='Input directory containing infilled tiles from MAT'
    )
    parser.add_argument(
        '--metadata_path',
        type=str,
...
def main():
    ...
    for sino_idx in tqdm(range(n_sinograms), desc="Merging sinograms"):
        ...
        # Crop to original shape (e.g., 410x3500 from padded 416x3504)
        h_start = top
    ...
    print(f"\nNext steps:")
    print(f"  python reconstruct_from_mat.py")
