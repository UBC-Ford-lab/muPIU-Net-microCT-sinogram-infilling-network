#!/usr/bin/env python3
"""
Reconstruction Pipeline for MAT-Infilled Sinograms
==================================================
This script:
1. Loads MAT-infilled (merged) sinogram PNGs using memory-mapped arrays (RAM-safe)
2. Denormalizes from [0, 65535] back to original value range using stored metadata
3. Reconstructs them back into projection format
4. Runs FDK reconstruction using existing pipeline
5. Saves reconstruction results

Normalization flow:
- merge_mat_tiles.py outputs NORMALIZED [0, 65535] uint16 PNGs
- This script denormalizes ONCE using per-sinogram min/max from metadata
- This matches the RePaint and DeepFill workflows exactly
...
Based on reconstruct_from_deepfill.py structure.

Author: Claude (Anthropic)
Date: 2025-12-01 (updated 2025-12-05: clarified normalization flow)
"""
...
def load_infilled_sinograms(sinogram_folder, metadata, memmap_path,
                           gt_sinogram_dir=None, mask_dir=None, force_reload=False):
    """
    ...
    Args:
        ...
        gt_sinogram_dir: Path to ground truth sinograms (for range scaling)
        mask_dir: Path to mask files (for determining valid pixels)
    ...
    Note:
        If gt_sinogram_dir and mask_dir are provided, sinograms are scaled to match
        the valid (non-masked) pixel range of the ground truth. This fixes issues
        with MAT outputting zero pixels that cause streak artifacts in FDK.
    """
    ...
    use_gt_scaling = gt_sinogram_dir is not None and mask_dir is not None
    if use_gt_scaling:
        gt_sinogram_dir = Path(gt_sinogram_dir)
        mask_dir = Path(mask_dir)
        print(f"Using GT-based scaling (fixes MAT zero pixels):")
        print(f"  GT sinograms: {gt_sinogram_dir}")
        print(f"  Masks: {mask_dir}")
    else:
        print(f"Using metadata-based normalization (original mode)")
    ...
    for idx, sino_file in enumerate(tqdm(sinogram_files, desc="Loading sinograms")):
        ...
        # Sinograms should already have padding removed by merge_mat_tiles.py
        img_unpadded = img
        ...
        if use_gt_scaling:
            # GT-based scaling: Scale MAT to match valid pixel range of ground truth
            # This fixes MAT zero pixels that cause streak artifacts in FDK
            gt_sino_path = gt_sinogram_dir / f'sino_{height_idx:04d}.png'
            mask_path = mask_dir / f'sino_{height_idx:04d}_mask001.png'

            gt_sinogram = np.array(Image.open(gt_sino_path))
            mask = np.array(Image.open(mask_path))

            # Get physical range from metadata for this sinogram
            sino_metadata = metadata['sinograms'][height_idx]
            meta_min = sino_metadata['normalization_gt']['min']
            meta_max = sino_metadata['normalization_gt']['max']

            # Convert GT sinogram from [0, 65535] to physical units
            gt_sino_physical = (gt_sinogram.astype(np.float32) / 65535) * (meta_max - meta_min) + meta_min

            # Get range from valid (non-masked) pixels only, now in PHYSICAL units
            # Mask convention: 0=inpaint (masked), 255=keep (valid)
            valid_pixels = gt_sino_physical[mask == 255]
            gt_min = float(valid_pixels.min())
            gt_max = float(valid_pixels.max())

            # Scale MAT from [0, 65535] to physical [gt_min, gt_max]
            img_denorm = (img_unpadded / max_value) * (gt_max - gt_min) + gt_min
        else:
            # Original metadata-based normalization
            sino_metadata = metadata['sinograms'][height_idx]
...
def reconstruct_from_mat_output(
    ...
    """
    Main reconstruction pipeline for MAT-infilled sinograms

    Args:
        infilled_folder: Path to MAT merged output sinograms
    ...
    print("="*70)
    print("MAT Sinogram Reconstruction Pipeline")
    print("="*70)
    ...
    if memmap_path is None:
        memmap_path = str(output_folder.parent / 'mat_projections_memmap.dat')
    ...
    projections_readonly = load_infilled_sinograms(
        infilled_folder, metadata, memmap_path,
        gt_sinogram_dir=gt_sinogram_dir,
        mask_dir=mask_dir,
        force_reload=force_reload
    )

    # Create a working copy for FDK (since it modifies data in-place)
    working_memmap_path = str(Path(memmap_path).parent / 'mat_projections_working.dat')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Reconstruct CT volumes from MAT-infilled sinograms'
    )
    parser.add_argument(
        '--sinogram_dir',
        type=str,
    ...
    parser.add_argument(
        ...
        help='Path to ground truth sinograms (for range scaling to fix zero pixels)'
    )
    parser.add_argument(
        '--mask_dir',
        type=str,
        default='../../../shared/sinogram_dataset/masks',
        help='Path to mask files (for determining valid pixels)'
    )
    parser.add_argument(
        '--no_gt_scaling',
        action='store_true',
        help='Disable GT-based scaling (use original metadata normalization)'
    )
    parser.add_argument(
        '--force_reload',
        action='store_true',
    ...
    # Handle paths - resolve relative paths relative to script directory, not cwd
    script_dir = Path(__file__).parent

    def resolve_path(p):
        """Resolve path: absolute paths unchanged, relative paths resolved from script dir."""
        path = Path(p)
        if path.is_absolute():
            return path
        return (script_dir / path).resolve()

    sinogram_dir = resolve_path(args.sinogram_dir)
    output_dir = resolve_path(args.output_dir)
    scan_folder = resolve_path(args.scan_folder)
    metadata_path = resolve_path(args.metadata_path)
    tiling_metadata_path = resolve_path(args.tiling_metadata_path)

    # Handle GT scaling paths (None if disabled)
    if args.no_gt_scaling:
        gt_sinogram_dir = None
        mask_dir = None
    else:
        gt_sinogram_dir = resolve_path(args.gt_sinogram_dir)
        mask_dir = resolve_path(args.mask_dir)

    # Reconstruct MAT infilled sinograms
    print("\n" + "="*80)
    print("RECONSTRUCTING MAT INFILLED")
    print("="*80)
    reconstruct_from_mat_output(
    ...
    print("="*80)
    print(f"MAT reconstruction: {output_dir}.vff")
